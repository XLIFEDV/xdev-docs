...
const ROOT_GROUPS = ["vehicles", "scripts"]; // docs/vehicles, docs/scripts
...
async function buildSidebarTree(docFiles) {
  ...
  // prettify category labels (top level = VEHICLES / SCRIPTS)
...
async function main() {
  ...
  const vehicles = await buildGroupLinks("vehicles");
  const scripts = await buildGroupLinks("scripts");

  const generated = { vehicles, scripts };
  ...
  console.log(`[gen-docs] vehicles: ${vehicles.length}, scripts: ${scripts.length}`);
}
